...
from pystyle import *
import webbrowser
webbrowser.open('https://t.me/MUSASHI_NO_WAT ') 
intro = """                                                                                                      
                        femoz
        
                 v1.0.1 by: @MUSASHI_TYRK
                      SNOS TOOL
...
text = """
                         ...
                         CRACK BY: @musashi         price: @MUSASHI_NO_WAT  
  ...
  └───────────────────────────┘      └───────────────────────────┘      └───────────────────────────┘
                                       ...
                                       ну а че взломал бывает = ) !                       
                                                                                                                                                                   
                     """
print()
print(Colorate.Vertical(Colors.green_to_red, Center.XCenter(banner)))
...
select = input(f'{COLOR_CODE["RED"]}☄ {COLOR_CODE["CYAN"]} НАЖМИ ENTER ЧТОБЫ НАЧАТЬ{COLOR_CODE["CYAN"]} ')
def menu():
    choice = input(f'{COLOR_CODE["RED"]}☠︎{COLOR_CODE["RED"]} Выбрать функцию ➤ {COLOR_CODE["RED"]} ')
    return choice
